...
import styles from './VolunteerOpenPosition.module.scss';
import { IVolunteerOpenPositionProps } from './IVolunteerOpenPositionProps';
...
export interface IVolunteerOpenPositionState {
  items: any[];
  showSpinner: boolean;
  hasNext: boolean;
  hasPrevious: boolean;
...
export default class VolunteerOpenPosition extends React.Component<IVolunteerOpenPositionProps, IVolunteerOpenPositionState> {
  constructor(props: IVolunteerOpenPositionProps) {
    super(props);
    this.state = {
      items: [],
      showSpinner: true,
  ...
  public render(): React.ReactElement<IVolunteerOpenPositionProps> {
    return (
    <div>
      { this.state.showSpinner ? <div className={styles.Spinner}><Spinner size={SpinnerSize.large} /></div>:
      <div className={ styles.volunteerOpenPosition }>
        ...
        <div className={styles.headerRow}>
          ...
          {this.state.items.map((item,key)=>{
            ...
            return (<div className={ rowClass }>
             <div className={styles.itemTitle}><Link href = {'https://christclinickaty.sharepoint.com/sites/intranet/volunteers/Lists/Volunteer%20Positions/DispForm.aspx?ID='+item.Id+"&Source="+ window.location.href}>{item.Title}</Link></div>
  ...
  private GetFirstPageData() {
    debugger;
    this.props.Web.lists.getByTitle("Volunteer Positions").items.filter("Status ne 'Closed'").top(10).getPaged()
      .then(response => {
  ...
  private handleApply = () => {
    debugger;
    window.location.href = "https://christclinickaty.sharepoint.com/sites/intranet/volunteers/Lists/Volunteer%20Position%20Application/NewForm.aspx?Source="+window.location.href;
  }
